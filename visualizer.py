...
def add_to_visualize_buffer(visualize_buffer, visualize_points, visualize_colors):
    assert (
        visualize_points.shape[0] == visualize_colors.shape[0]
    ), f"got {visualize_points.shape[0]} for points and {visualize_colors.shape[0]} for colors"
...
class Visualizer:
    ...
    def show_pointcloud(self, points, colors):
        ...
        )  # float64 is a lot faster than float32 when added to o3d later
    ...
    def _get_scene_points_and_colors(self):
        ...
        for cam_id in range(len(cam_obs)):
            ...
            # clip to workspace
            within_workspace_mask = filter_points_by_bounds(
                cam_points, self.bounds_min, self.bounds_max, strict=False
            )
    ...
    def visualize_subgoal(self, subgoal_pose):
        visualize_buffer = {"points": [], "colors": []}
        # scene
        ...
        collision_points_centered = (
            np.dot(collision_points, centering_transform[:3, :3].T)
            + centering_transform[:3, 3]
        )
        transformed_collision_points = batch_transform_points(
            collision_points_centered, subgoal_pose_homo[None]
        ).reshape(-1, 3)
        collision_points_colors = np.array([self.color] * len(collision_points))
        add_to_visualize_buffer(
            visualize_buffer, transformed_collision_points, collision_points_colors
        )
        # add keypoints
        ...
        keypoints_colors = [
            color_map(i / num_keypoints)[:3] for i in range(num_keypoints)
        ]
        for i in range(num_keypoints):
            nearby_points = generate_nearby_points(
                keypoints[i], num_points_per_side=6, half_range=0.009
            )
    ...
    def visualize_path(self, path):
        ...
        for t in range(len(path) - 1):
            ...
            interp_colors = (
                1 - whitening_coef
            ) * interp_colors + whitening_coef * np.array([1, 1, 1])
        ...
        poses_homo = T.convert_pose_quat2mat(
            path[:, :7]
        )  # the last number is gripper action
        transformed_collision_points = batch_transform_points(
            collision_points_centered, poses_homo
        ).reshape(-1, 3)  # (num_poses, num_points, 3)
        # calculate color based on the timestep
        collision_points_colors = (
            np.ones([path_length, num_points, 3]) * self.color[None, None]
        )
        for t in range(path_length):
            whitening_coef = 0.3 + 0.5 * (t / path_length)
            collision_points_colors[t] = (1 - whitening_coef) * collision_points_colors[
                t
            ] + whitening_coef * np.array([1, 1, 1])
