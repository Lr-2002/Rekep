...
class Main:
    def __init__(self, scene_file, visualize=False):
        ...
        ), "The IK solver assumes the robot is a Fetch robot"
        ik_solver = IKSolver(
            robot_description_path=self.env.robot.robot_arm_descriptor_yamls[
                self.env.robot.default_arm
            ],
    ...
    def perform_task(self, instruction, rekep_program_dir=None, disturbance_seq=None):
        ...
        points = cam_obs[self.config["vlm_camera"]]["points"]  # PC
        mask = cam_obs[self.config["vlm_camera"]]["seg"]  # from maniskill
        print(f"the shape of rgb and points is {rgb.shape} and {points.shape}")
        # ====================================
    ...
    def _execute_grasp_action(self):
        ...
        grasp_pose[:3] += T.quat2mat(pregrasp_pose[3:]) @ np.array(
            [self.config["grasp_depth"], 0, 0]
        )
        grasp_action = np.concatenate(
            [grasp_pose, [self.env.get_gripper_close_action()]]
        )
...
if __name__ == "__main__":
    ...
    if args.apply_disturbance:
        assert (
            args.task == "pen" and args.use_cached_query
        ), "disturbance sequence is only defined for cached scenario"
    ...
    task_list = {
        "pen": {
            "scene_file": "./configs/og_scene_file_clean_desk.json",
            "instruction": "Put the pen in the middle of the table",
